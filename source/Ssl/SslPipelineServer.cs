...
namespace Rrs.Ssl
{
    ...
    public class SslPipelineServer : TcpPipelineServer
    {
        ...
        protected override void OnCreatePipeline<TState>(Socket socket, ConnectCallback<TState> callback, TState state)
        {
            var pipeline = new SslPipeline(socket);
            pipeline.AuthenticateConnection(certificate, callback, state);
        }
